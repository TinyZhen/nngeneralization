...
def two_moons():
  ...
  config.add_argument("--data", type=str, choices=["two_moons","mnist"], default="mnist")
  ...
  val_data = datasets.MNIST(config.data_root, transform=transforms.ToTensor(), train=False, download=True)
  val_loader = torch.utils.data.DataLoader(val_data, batch_size=config.batch_size, shuffle=True, num_workers=config.workers)
  ...
  logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('log/freeze/%s_%s_%s.txt' % (config.data, config.model,datetime.now())),  # Specify your log file name/path
            logging.StreamHandler()
        ]
    )
  polytope_sample_counts = {}

  # Loop to count samples in each polytope
  for batch_i, (data, targets) in enumerate(test_loader):
      ...
      if config.method == "subfunctions":
          unreliability_i, corrects_i, polytopes_i = globals()["%s_metric" % config.method](
              config, method_variables, model, inputs, targets, get_polytope_ids=True)
      else:
          raise NotImplementedError
  ...
  polytope_img = np.zeros((num_test_side, num_test_side, 3), dtype=np.uint8)
  unreliability_img = np.zeros((num_test_side, num_test_side), dtype=np.float64)
...
#  number of samples
      color = polytope_colours[polytope_str]
      assert (inds[j][0] == int(inds[j][0]) and inds[j][1] == int(inds[j][1]))

      # Flip the axes - x axis is actually the second dimension of the image
      polytope_img[int(inds[j][1]), int(inds[j][0]), :] = color
...
# for random color
  ...
  log_and_print("unreliability range: %s, %s" % (unreliability_img.max(), unreliability_img.min()))
  ...
  # fig, ax = plt.subplots(1, figsize=(4, 4))
  # im = ax.imshow(polytope_img, origin="lower", interpolation="antialiased")
  # ax.set_xticks([])
  # ax.set_yticks([])
  # ax.set_xlabel("Input dim 0", fontsize=14)
  # ax.set_ylabel("Input dim 1", fontsize=14)
  ...
  # num_labels = 5  # Choose the number of labels you want to display
  # step = max(1, len(unique_counts) // num_labels)
  # selected_counts = unique_counts[::step]


  # cbar = fig.colorbar(cm.ScalarMappable(cmap='viridis'), ax=ax, orientation='vertical')
  # cbar.set_ticks(np.linspace(0, 1, len(selected_counts)))
  # cbar.set_ticklabels(selected_counts)
  ...
  # plt.text(0.5, 1.05, f"Total Samples: {total_samples}", fontsize=14, ha='center', transform=ax.transAxes)

  # fig.tight_layout()
  # fig.savefig(osp.join(config.models_root, "%s.png" % cleanstr("%s_results_2_%s" % (save_prefix,datetime.now()))))
  ...
  print("corners:")
  log_and_print((unreliability_img[0, 0],
  ...
  #                interpolation="antialiased", vmin = 0, vmax = 100)  # axis from 0 to num_test_side - 1
  ...
  # fig.tight_layout()
  # # plt.autoscale()
  # figstr = osp.join(config.models_root, "%s.png" % cleanstr("%s_results_4_%s" % (save_prefix,datetime.now())))
  # fig.savefig(figstr)

  # plt.close("all")
  print("Saved to: %s*" % osp.join(config.models_root, save_prefix))
  delete_files('two_moons_4_subfunctions_mlp__method_variables_p_*.pytorch')  
