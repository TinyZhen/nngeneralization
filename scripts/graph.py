    ...
    with open(log_file, 'r') as file:
        for line in file:
            ...
            if "polytope colours sz" in line:
                ...
                aasr_value = float(re.search(r'Average AASR: ([\d\.]+)', line).group(1))
                aasr.append(aasr_value)
            elif "std" in line:
                std_value = float(re.search(r'std: ([\d\.]+)', line).group(1))
...
def save_file(df,train_loss,test_loss,i,mode):
    ...
    combined_df.to_csv(f'dataframe{i}_{mode}_{sz}x{sz}_{combined_df.loc[1,"seed"]}.csv')
    return combined_df


base_root = "log/base/cifar10/tmp_3"

free_root = "log/freeze/cifar10/tmp_3"
...
for subdir, dirs, files in os.walk(base_root):
    ...
    for dir_name in dirs:
        # if dir_name == '179665917':
                dir_path = os.path.join(subdir, dir_name,f"[{sz}, {sz}]")
    ...
    for dir_name in dirs:
        ...
        if dir_name in base_dirs:
            # if dir_name == '179665917':
                ...
                free_dir_path = os.path.join(subdir, dir_name,f"[{sz}, {sz}]")
                print(base_dir_path)
                print(free_dir_path)
                ...
                if base_files:
                    ...
                    base_writer = pd.ExcelWriter(f'cor_dataframe_base_{sz}x{sz}_{base_combined_df.loc[1,"seed"]}.xlsx', engine='xlsxwriter')
                    ...
                    fig.savefig(f'combined_plot_{sz}x{sz}_{base_combined_df.loc[1,"seed"]}.png')
                    i += 1
                    base_writer.close()
    break
