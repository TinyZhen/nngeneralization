...
def subfunctions_pre(config, model, train_loader, val_loader):
  ...
  if config.precompute:
    ...
    metric_values = np.zeros(len(config.search_ps), dtype=np.float64)
...
def STEB(config, query_patterns, method_variables, deltas):
  ...
  if not config.no_bound:
    # print(f'{method_variables["global_prob_norm"]}')
    ...
    smooth_prob = torch.clamp(smooth_prob, min=epsilon)
    delta_log = torch.log(2. / deltas.unsqueeze(0))
    # print("delta_log:", delta_log)
    
    gen_gap_bound = torch.sqrt(delta_log / (2. * float(method_variables["m"]))) / smooth_prob.unsqueeze(1)
    
    if torch.any(torch.isnan(gen_gap_bound)) or torch.any(torch.isinf(gen_gap_bound)):
        print("Warning: gen_gap_bound has NaN or Inf values")
        print("gen_gap_bound:", gen_gap_bound)
        print("smooth_prob:", smooth_prob)
        print("delta_log:", delta_log)
...
def compute_closeness(config, query_patterns, method_variables):
  ...
  for batch_l in range(num_batches):
    start_l, end_l = batch_l * method_variables["pattern_batch_sz"], min(num_pop_polytopes,
                                                                         ...
                                                                         method_variables[
                                                                           "pattern_batch_sz"])
    print(f"number of polytype: {num_pop_polytopes} and number of batch: {batch_l}")
    print(f"Shape of train_ind_to_patt: {method_variables['train_ind_to_patt'].shape}")

    patterns = method_variables["train_ind_to_patt"][start_l: end_l,
               :]  # batch sz or less, patt len
    if query_patterns.size() > patterns.size():
      print("query patter shape:  " + str(query_patterns.size()) + "pattern shape"+  str(patterns.size()))
  ...
  for i, (name, m) in enumerate(model.named_children()):  # ordered
    ...
    for k_info in keywords:
      if fits_keyword(m, full_name, k_info):
        ...
        found = True
        print(f"Layer: {full_name}, recorded shape: {m.recorded.shape}")
...
def compute_metric_per_delta(config, unreliability_per_param, corrects):
  ...
  tprs, fprs = np.zeros((config.threshold_divs + 1 + 2, num_search_params), dtype=np.float64), \
               np.zeros((config.threshold_divs + 1 + 2, num_search_params), dtype=np.float64)
  coverages = np.zeros((config.threshold_divs + 1 + 2, num_search_params), dtype=np.float64)
  acc_or_precisions = np.zeros((config.threshold_divs + 1 + 2, num_search_params), dtype=np.float64)
  ...
  results = np.zeros(num_search_params, dtype=np.float64)
  for i in range(num_search_params):
