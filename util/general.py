...
def eval_and_store(config, unreliability, corrects, method_variables, store_fname_prefix):
  ...
  tprs, fprs = np.zeros(config.threshold_divs + 1 + 2, dtype=float), np.zeros(
    config.threshold_divs + 1 + 2, dtype=float)
  coverages = np.zeros(config.threshold_divs + 1 + 2, dtype=float)
  effective_acc_or_precisions = np.zeros(config.threshold_divs + 1 + 2, dtype=float)
  recalls = np.zeros(config.threshold_divs + 1 + 2, dtype=float)
